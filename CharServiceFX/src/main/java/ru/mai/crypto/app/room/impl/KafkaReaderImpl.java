...
public class KafkaReaderImpl implements KafkaReader {
    ...
    public KafkaReaderImpl(Client client, int roomId, BigInteger[] keyParameters) {
        this.client = client;
        this.kafkaConsumer = new KafkaConsumer<>(
                Map.of(
                        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9093",
                        ConsumerConfig.GROUP_ID_CONFIG, "consumer_" + roomId + "_" + client.getClientId(),
                        ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest"
                ),
                new ByteArrayDeserializer(),
                new ByteArrayDeserializer()
        );
        log.info("{} user listen {} topic", client.getClientId(), "input_" + roomId + "_" + client.getClientId());
        kafkaConsumer.subscribe(Collections.singletonList("input_" + roomId + "_" + client.getClientId()));
            ...
            while (isRunning) {
                ...
                for (ConsumerRecord<byte[], byte[]> consumerRecord : consumerRecords) {
                    ...
                    if (jsonMessage.contains("cipherInfo")) {
                        ...
                        if (cipher != null) {
                            String test = new String(consumerRecord.value());
                            Message message = parseMessage(test);

                            if (message != null && message.getBytes() != null) {
                                log.info("User get message: {}", new String(cipher.decrypt(message.getBytes())));
                            }
                        }
                    }
                }
