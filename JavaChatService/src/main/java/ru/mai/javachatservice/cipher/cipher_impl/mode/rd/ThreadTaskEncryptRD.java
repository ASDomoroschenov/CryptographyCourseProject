package ru.mai.javachatservice.cipher.cipher_impl.mode.rd;
...
import ru.mai.javachatservice.cipher.utils.BitsUtil;

@AllArgsConstructor
public class ThreadTaskEncryptRD implements TextThreadTask {
    private final CipherService cipherService;
    private final byte[][] counterBlocks;

    public ThreadTaskEncryptRD(CipherService cipherService, byte[] text, byte[] initialVector) {
    ...
    public Pair<Integer, byte[]> apply(byte[] text, int indexBegin, int textBlockSize, int countBlocks) {
        ...
        for (int i = 0; i < countBlocks; i++) {
            ...
            byte[] cipherBlockText = cipherService.encryptBlock(BitsUtil.xor(textBlock, counterBlocks[(indexBegin + i * textBlockSize) / textBlockSize]));
